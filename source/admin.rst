...
On the server where you plan to deploy GeoServices, create the ``/srv/geoservices`` directory, then go to it, download the configuration template (`docker-compose-2.20.0.tar.bz2 <https://nextgis.com/onpremise/geoservices/docker-compose-2.20.0.tar.bz2>`_, where 2.20.0 is the current version) and unpack it. If the server does not have Internet access, download the file on another PC and transfer it to the server.
