...
Откройте группу сервисов, в которой хотите создать сервис базовой карты, нажмите **Создать сервис**.

.. figure:: _static/geosop_ngw_create_button_ru.png
   :name: geosop_base_create_button_pic
   :align: center
   :width: 20cm
...
Сервис NextGIS Web позволяет создавать кэшированные тайловые сервисы на основе веб-карт, созданных в NextGIS Web. 

После того, как тайлы записаны в кеш, обращение к созданному сервису не затрагивает NextGIS Web, таким образом можно создать сервис для высоких пиковых нагрузок и снизить нагрузку на сам NextGIS Web.
...
Чтобы создать сервис на основе веб-карты, зайдите в группу, где он должен размещаться, и нажмите **Создать сервис**.
...
* название сервиса;
* диапазон масштабных уровней для кэширования;
* выберите NextGIS Web, подключённую к GeoServices;
* выберите ресурс веб-карты.
...
.. important:: Обратите внимание, что у внешних сервисов могут быть разные условия использования. Ознакомьтесь с ними, прежде чем создавать сервисы на их основе. Нарушение условий использования может привести к блокировке со стороны источника данных.

Перейдите в нужную группу сервисов и нажмите **Создать сервис**.
...
* название, 
* диапазон уровней отображения, 
* границы сервиса, 
* URL добавляемого TMS сервиса, 
* система координат.
...
После этого сервис появится в списке соответствующей группы. При необходимости сервис можно отредактировать или удалить.
...
Вы можете создать сервис из готового набора тайлов. Например, загрузить тайлы базовой карты, `заказанные на NextGIS Data <https://data.nextgis.com/ru/region/custom/tiles/>`_.

Перейдите в нужную группу сервисов и нажмите **Создать сервис**.

.. figure:: _static/geosop_ngw_create_button_ru.png
   :name: geosop_tiles_create_button_pic
   :align: center
   :width: 20cm

   Кнопка создания нового сервиса

Достаточно задать название для сервиса и загрузить архив с тайлами, остальные параметры будут установлены автоматически.

.. figure:: _static/geosop_tiles_new_param_ru.png
   :name: geosop_tiles_new_param_pic
   :align: center
   :width: 16cm

   Создание сервиса на основе набора тайлов
